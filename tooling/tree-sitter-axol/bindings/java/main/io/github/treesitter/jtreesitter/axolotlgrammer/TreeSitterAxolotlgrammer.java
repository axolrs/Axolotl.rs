package io.github.treesitter.jtreesitter.axolotlgrammer;

import java.lang.foreign.*;

public final class TreeSitterAxolotlgrammer {
    ...
    private static final TreeSitterAxolotlgrammer INSTANCE = new TreeSitterAxolotlgrammer();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterAxolotlgrammer() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_axolotlgrammer");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-axolotlgrammer");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-axolotlgrammer");
